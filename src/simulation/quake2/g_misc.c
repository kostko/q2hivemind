...
// -- FEAR Begin --
	cvar_t *allowed = gi.cvar( "teleport", "1", 0 );
	if (!allowed->value)
		return;
// -- FEAR End --
...
//ZOID
 ...
 // -- FEAR Begin --
	if (other->client->botinfo.active)
	{
		VectorCopy( dest->s.origin, other->client->botinfo.prev_origin );
		VectorCopy( dest->s.angles, other->client->botinfo.prev_angles );
